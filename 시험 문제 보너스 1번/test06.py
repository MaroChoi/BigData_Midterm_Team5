...
df = pd.read_csv('C:\BigData_Midterm_Team5\BigData_Midterm_Team5\시험 문제 보너스 1번\6_shopping.csv')
# 1단계: 고유값 확인
...
ordinal_numeric_cols = []
nominal_numeric_cols = ['Customer ID']
ordinal_string_cols = []
nominal_string_cols = ['Invoice', 'StockCode', 'Description', 'InvoiceDate', 'Country']
...
selected_columns = ['Invoice', 'StockCode', 'Description', 'Quantity', 'InvoiceDate', 'Price', 'Customer ID', 'Country']
df_selected = df[selected_columns]
...
df_selected['TotalAmount'] = df_selected['Quantity'] * df_selected['Price']
...
# 새로 만든 파생변수 추가
numerical_cols_selected.append('TotalAmount')
